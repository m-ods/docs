...
3. 3Create a new `RealtimeTranscriber` using the function you created.\`\`\`
   transcriber = aai.RealtimeTranscriber(    sample\_rate=16\_000,    on\_data=on\_data,    on\_error=on\_error,    on\_open=on\_open,    on\_close=on\_close,)```\<Note>
...
1. 1Open a microphone stream. The `sample_rate` needs to be the same value as the one you passed to `RealtimeTranscriber`.\`\`\`
   microphone\_stream = aai.extras.MicrophoneStream(sample\_rate=16\_000)```\<Note>
